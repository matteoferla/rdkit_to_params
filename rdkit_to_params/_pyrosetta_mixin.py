...
class _PoserMixin:
    def test(self, outfile: Optional[str] = None) -> pyrosetta.Pose:
        ...
        """
        self.log.debug(
            f"Testing params {'(writing out as ' + outfile + ')' if outfile is not None else '(no saving)'}"
        )
    ...
    def to_pose(self, relax: bool = False) -> pyrosetta.Pose:
        pose = pyrosetta.Pose()
        # add paramsblock
    ...
    def add_residuetype(
        self, pose: "pyrosetta.Pose", reset: bool = True
    ) -> prc.chemical.ResidueTypeSet:
        """
        ...
        new = pyrosetta.rosetta.core.chemical.read_topology_file(stream, self.NAME, rts)
